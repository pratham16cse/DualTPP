...
def get_processed_data(dataset_name, args):
	...
	itv = [train_times_timestamps[0], train_times_timestamps[-1]]
	model.fit(train_times_timestamps,itv)
	...
	os.makedirs('Outputs/hawkes_model', exist_ok=True)
	model.plot_N_pred()
	plt.savefig('Outputs/hawkes_model/count_pred.png')
	plt.close()
	model.plot_KS()
	plt.savefig('Outputs/hawkes_model/KS_plot.png')
	plt.close()

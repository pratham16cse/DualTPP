...
def get_end_time_from_bins_comp(test_inp_times_in_bin, test_end_hr_bins, enc_len=20, comp_bin_sz=10):
	test_data_in_gaps_bin_lst = list()
	for idx in range(len(test_end_hr_bins)):
...
def get_processed_data(dataset_name, args):
	...
	train_times_timestamps_norm = np.cumsum(train_times_gaps_norm) + (train_times_timestamps[0]-train_times_gaps[0])
	test_times_gaps_norm = normalize_avg_given_param(test_times_gaps, gaps_norm_a, gaps_norm_d)
	test_times_timestamps_norm = np.cumsum(test_times_gaps_norm) + (train_times_timestamps_norm[-1])
	...
	test_times_gaps_norm = hawkes_timestamps_pred - np.concatenate([train_times_timestamps_norm[-1:], hawkes_timestamps_pred[:-1]])
	...
	train_data_in_gaps_comp, train_data_out_gaps_comp, _, _, _ = \
	make_seq_from_data(train_times_gaps_comp, comp_enc_len, in_bin_sz, out_bin_sz, batch_size, False, dataset_name=dataset_name)
	...
	train_data_in_gaps_comp, train_norm_a_gaps_comp, train_norm_d_gaps_comp = normalize_avg(train_data_in_gaps_comp)
	train_data_out_gaps_comp = normalize_avg_given_param(train_data_out_gaps_comp, train_norm_a_gaps_comp, train_norm_d_gaps_comp)

	[train_data_in_gaps_comp, train_data_out_gaps_comp, dev_data_in_gaps_comp, dev_data_out_gaps_comp,
	...
	[train_data_in_gaps_comp, train_data_out_gaps_comp, train_data_in_timestamps_comp, train_data_out_timestamps_comp] = \
	stabalize_data( train_data_in_gaps_comp, train_data_out_gaps_comp, train_data_in_timestamps_comp, 
					train_data_out_timestamps_comp, batch_size)
	...
	# TODO: Can check for max possible enc_len

...
class RMTPP_DECRNN:
    ...
    def predict(self, event_in_seq, time_in_seq, decoder_length, plt_tru_gaps, single_threaded=False, is_plot=False):
        ...
        def _quad_worker(params):
            ...
            for pred_idx, s_i in enumerate(all_decoder_states):
                ...
                if is_plot:
                    ...
                    plt.ylabel('Density')
                    plt.legend(loc='best')
                    plt.savefig('DensityPlots/'+is_plot+'/instance_'+str(batch_idx)+'.png')
                    plt.close()
...
#        if single_threaded:
        all_time_preds = [_quad_worker((idx, (state, h_m, t_last, tru_gap))) for idx, (state, h_m, t_last) in enumerate(zip(all_decoder_states, cur_state, time_pred_last, plt_tru_gaps))]
#        else:

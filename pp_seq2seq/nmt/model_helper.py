...
def create_train_model(
  ...
  src_event_file = "%s.%s.%s" % (hparams.train_prefix, 'event', hparams.src)
  src_time_file = "%s.%s.%s" % (hparams.train_prefix, 'time', hparams.src)
  tgt_event_file = "%s.%s.%s" % (hparams.train_prefix, 'event', hparams.tgt)
  tgt_time_file = "%s.%s.%s" % (hparams.train_prefix, 'time', hparams.tgt)
  ...
  with graph.as_default(), tf.container(scope or "train"):
    ...
    src_event_dataset = tf.data.TextLineDataset(tf.gfile.Glob(src_event_file))
    src_time_dataset = tf.data.TextLineDataset(tf.gfile.Glob(src_time_file))
    tgt_event_dataset = tf.data.TextLineDataset(tf.gfile.Glob(tgt_event_file))
    tgt_time_dataset = tf.data.TextLineDataset(tf.gfile.Glob(tgt_time_file))
...
class EvalModel(
    collections.namedtuple("EvalModel",
                           ("graph", "model", "src_mark_file_placeholder", "src_time_file_placeholder",
                            "tgt_mark_file_placeholder", "tgt_time_file_placeholder", "iterator"))):
  pass
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "eval"):
    ...
    src_mark_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    src_time_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    tgt_mark_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    tgt_time_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    src_mark_dataset = tf.data.TextLineDataset(src_mark_file_placeholder)
    src_time_dataset = tf.data.TextLineDataset(src_time_file_placeholder)
    tgt_mark_dataset = tf.data.TextLineDataset(tgt_mark_file_placeholder)
    tgt_time_dataset = tf.data.TextLineDataset(tgt_time_file_placeholder)
    iterator = iterator_utils.get_iterator(
        src_mark_dataset,
        src_time_dataset,
        tgt_mark_dataset,
  ...
  return EvalModel(
      graph=graph,
      model=model,
      src_mark_file_placeholder=src_mark_file_placeholder,
      src_time_file_placeholder=src_time_file_placeholder,
      tgt_mark_file_placeholder=tgt_mark_file_placeholder,
      tgt_time_file_placeholder=tgt_time_file_placeholder,
      iterator=iterator)
...
class InferModel(
    collections.namedtuple("InferModel",
                           ("graph", "model",
                            "src_mark_placeholder", "src_time_placeholder",
...
def create_infer_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "infer"):
    ...
    src_mark_placeholder = tf.placeholder(shape=[None], dtype=tf.string)
    src_time_placeholder = tf.placeholder(shape=[None], dtype=tf.string)
    batch_size_placeholder = tf.placeholder(shape=[], dtype=tf.int64)

    src_mark_dataset = tf.data.Dataset.from_tensor_slices(
        src_mark_placeholder)
    src_time_dataset = tf.data.Dataset.from_tensor_slices(
        src_time_placeholder)
  ...
  return InferModel(
      graph=graph,
      model=model,
      src_mark_placeholder=src_mark_placeholder,
      src_time_placeholder=src_time_placeholder,

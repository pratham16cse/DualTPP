...
def add_arguments(parser):
  ...
  parser.add_argument("--inference_input_time_file", type=str, default=None,
                      help="Set to the time file to decode.")
  parser.add_argument("--inference_list", type=str, default=None,
                      help=("A comma-separated list of sentence indices "
                            "(0-based) to decode."))
  parser.add_argument("--infer_batch_size", type=int, default=32,
                      help="Batch size for inference mode.")
  parser.add_argument("--inference_output_mark_file", type=str, default=None,
                      help="Output file to store marker decoding results.")
  parser.add_argument("--inference_output_time_file", type=str, default=None,
                      help="Output file to store time decoding results.")
  parser.add_argument("--inference_ref_mark_file", type=str, default=None,
                      help=("""\
      Reference file to compute evaluation scores of markers (if provided).\
      """))
  parser.add_argument("--inference_ref_time_file", type=str, default=None,
                      help=("""\
      Reference file to compute evaluation scores of times (if provided).\
      """))
...
def run_main(flags, default_hparams, train_fn, inference_fn, target_session=""):
  ...
  ## Train / Decode
  if flags.inference_input_mark_file and flags.inference_input_time_file:
    # Inference output directory
    trans_mark_file = flags.inference_output_mark_file
    trans_time_file = flags.inference_output_time_file
    assert trans_mark_file
    assert trans_time_file
    trans_dir = os.path.dirname(trans_mark_file)
    ...
    inference_fn(ckpt,
                 flags.inference_input_mark_file,
                 flags.inference_input_time_file,
                 flags.inference_output_mark_file,
                 flags.inference_output_time_file,
                 trans_file, hparams, num_workers, jobid)

    # Evaluation
    ref_mark_file = flags.inference_ref_mark_file
    ref_time_file = flags.inference_ref_time_file
    if ref_mark_file and tf.gfile.Exists(trans_mark_file) \
        and ref_time_file and tf.gfile.Exists(trans_time_file):
      for metric in hparams.metrics:

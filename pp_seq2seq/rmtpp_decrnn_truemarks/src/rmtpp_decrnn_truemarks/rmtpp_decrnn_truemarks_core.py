...
def_opts = Deco.Options(
    ...
    seed=42,
    scope='RMTPP_DECRNN',
    alg_name='rmtpp_decrnn_truemarks',
    save_dir='./save.rmtpp_decrnn_truemarks/',
    summary_dir='./summary.rmtpp_decrnn_truemarks/',
...
class RMTPP_DECRNN:
        ...
        with tf.variable_scope(scope):
            with tf.device(device_gpu if not cpu_only else device_cpu):
                ...
                with tf.name_scope('Decoder'):
                    ...
                    if self.ALG_NAME in ['rmtpp_decrnn_truemarks_wcmpt', 'rmtpp_decrnn_truemarks_mode_wcmpt']:
                    ...
                    elif self.ALG_NAME in ['rmtpp_decrnn_truemarks', 'rmtpp_decrnn_truemarks_mode']:
                        WT = self.wt
                    elif self.ALG_NAME in ['rmtpp_decrnn_truemarks_whparam', 'rmtpp_decrnn_truemarks_mode_whparam']:
                        WT = self.wt_hparam
            ...
            if self.STOP_CRITERIA == 'per_epoch_val_err' and epoch >= self.PATIENCE:
                ...
                if eval_train_data:
                    ...
                    train_time_preds, train_event_preds = self.predict(training_data['train_event_in_seq'],
                                                                   ...
                                                                   training_data['train_event_out_seq'],
    ...
    def predict(self, event_in_seq, time_in_seq, event_out_seq, decoder_length, plt_tru_gaps, single_threaded=False, plot_dir=False):
        ...
        def _quad_worker(params):
            ...
            for pred_idx, s_i in enumerate(decoder_states):
                ...
                if self.ALG_NAME in ['rmtpp_decrnn_truemarks', 'rmtpp_decrnn_truemarks_wcmpt', 'rmtpp_decrnn_truemarks_whparam']:
                    args = (c_, WT)
                    val, _err = quad(quad_func, 0, np.inf, args=args)
                    #print(batch_idx, D, c_, WT, val)
                elif self.ALG_NAME in ['rmtpp_decrnn_truemarks_mode', 'rmtpp_decrnn_truemarks_mode_wcmpt', 'rmtpp_decrnn_truemarks_mode_whparam']:

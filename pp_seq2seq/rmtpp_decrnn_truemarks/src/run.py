...
def_opts = rmtpp_decrnn_truemarks.rmtpp_decrnn_truemarks_core.def_opts
...
def cmd(dataset_name, alg_name, dataset_path,
    ...
    data = rmtpp_decrnn_truemarks.utils.read_seq2seq_data(
    ...
    def model_creator(params):
        ...
        #rmtpp_decrnn_truemarks.utils.data_stats(data) #TODO(PD) Need to support seq2seq models.

        hidden_layer_size, wt_hparam, restart, num_epochs, save_dir, train_eval = params
        rmtpp_decrnn_truemarks_mdl = rmtpp_decrnn_truemarks.rmtpp_decrnn_truemarks_core.RMTPP_DECRNN(
            sess=sess,
            ...
            _opts=rmtpp_decrnn_truemarks.rmtpp_decrnn_truemarks_core.def_opts
        )

        return rmtpp_decrnn_truemarks_mdl

    def hyperparameter_worker(params, rmtpp_decrnn_truemarks_mdl, dec_len, restore_path=None):
        ...
        # saving the model.
        rmtpp_decrnn_truemarks_mdl.initialize(finalize=False)
        result = rmtpp_decrnn_truemarks_mdl.train(training_data=data, restart=restart,
    ...
    for dec_len in decoder_length_run:
        ...
        if os.path.isfile(save_dir+'/result.json'):
            ...
            rmtpp_decrnn_truemarks_mdl = model_creator(args)
            result = hyperparameter_worker(args, rmtpp_decrnn_truemarks_mdl, dec_len)
        else:
            ...
            for params in product(*hparams[alg_name].values()):
                ...
                rmtpp_decrnn_truemarks_mdl = model_creator(args)
                result = hyperparameter_worker(args, rmtpp_decrnn_truemarks_mdl, dec_len, checkp_dir)

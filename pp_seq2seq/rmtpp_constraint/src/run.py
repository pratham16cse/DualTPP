...
def_opts = rmtpp_constraint.rmtpp_constraint_core.def_opts
...
def cmd(dataset_name, alg_name,
    ...
    data = rmtpp_constraint.utils.read_seq2seq_data(
    ...
    def hyperparameter_worker(params):
        ...
        #rmtpp_constraint.utils.data_stats(data) #TODO(PD) Need to support seq2seq models.

        hidden_layer_size, restart, num_epochs, save_dir = params
        rmtpp_constraint_mdl = rmtpp_constraint.rmtpp_constraint_core.RMTPP(
            sess=sess,
            ...
            cpu_only=cpu_only,
            _opts=rmtpp_constraint.rmtpp_constraint_core.def_opts
        )
        ...
        # saving the model.
        rmtpp_constraint_mdl.initialize(finalize=False)
        result = rmtpp_constraint_mdl.train(training_data=data, restart=restart,
                                 with_summaries=summary_dir is not None,
                                 num_epochs=num_epochs, with_evals=True)
        # del rmtpp_constraint_mdl
        return result

#!/usr/bin/env python
import click
import rmtpp_decrnn_mode_whparam_regu
...
l2_penalty_list = [0.001, 0.01, 0.1]
hparams = json.loads(open('hparams.json', 'r').read())

def_opts = rmtpp_decrnn_mode_whparam_regu.rmtpp_decrnn_mode_whparam_regu_core.def_opts
...
def cmd(dataset_name, alg_name,
    ...
    data = rmtpp_decrnn_mode_whparam_regu.utils.read_seq2seq_data(
    ...
    def hyperparameter_worker(params):
        ...
        #rmtpp_decrnn_mode_whparam_regu.utils.data_stats(data) #TODO(PD) Need to support seq2seq models.

        hidden_layer_size, w, l2_penalty, restart, num_epochs, save_dir = params
        rmtpp_decrnn_mode_whparam_regu_mdl = rmtpp_decrnn_mode_whparam_regu.rmtpp_decrnn_mode_whparam_regu_core.RMTPP_DECRNN(
            sess=sess,
            num_categories=data['num_categories'],
            hidden_layer_size=hidden_layer_size, # A hyperparameter
            wt=w, # A hyperparameter
            l2_penalty=l2_penalty, # A hyperparameter
            save_dir=save_dir,
            ...
            cpu_only=cpu_only,
            _opts=rmtpp_decrnn_mode_whparam_regu.rmtpp_decrnn_mode_whparam_regu_core.def_opts
        )
        ...
        # saving the model.
        rmtpp_decrnn_mode_whparam_regu_mdl.initialize(finalize=False)
        result = rmtpp_decrnn_mode_whparam_regu_mdl.train(training_data=data, restart=restart,
    ...
    if os.path.isfile(save_dir+'/result.json'):
        ...
        result = hyperparameter_worker((result['hidden_layer_size'], result['wt'], result['l2_penalty'], True, 0, result['checkpoint_dir']))
    else:
        ...
        if dataset_name == 'data_bookorder_40_1_20_20_20':
            hparams[alg_name]['wt'] = [3.62, 3.72, 3.82, 3.92]
        elif dataset_name == 'data_so_40_1_20_20_20':
            hparams[alg_name]['wt'] = [2.72, 2.82, 2.92, 3.02]

...
def_opts = Deco.Options(
    ...
    Wdt=lambda hidden_layer_size: 0.001*np.ones((1, hidden_layer_size)),
...
class RMTPP_DECRNN_PRE:
    ...
    def __init__(self, sess, num_categories, params_named, params_alias_named, hidden_layer_size, batch_size,
                 ...
                 Wt, Wem, Wh, bh, Ws, bs, wt, Wy, Vy, Vt, Vw, bk, bt, bw, wt_hparam, Wdt,
        ...
        with tf.variable_scope(scope):
            with tf.device(device_gpu if not cpu_only else device_cpu):
                ...
                with tf.variable_scope('decoder_state'):
                    self.Wdt = tf.get_variable(name='Wdt',
                                              shape=(1, self.HIDDEN_LAYER_SIZE),
                                              dtype=self.FLOAT_TYPE,
                                              regularizer=self.RNN_REGULARIZER,
                                              initializer=tf.constant_initializer(Wdt(self.HIDDEN_LAYER_SIZE)))
                ...
                self.all_vars = [self.Wt, self.Wem, self.Wh, self.bh, self.Ws, self.bs, self.Wdt,
                ...
                self.initial_state = state = tf.zeros([self.inf_batch_size, self.HIDDEN_LAYER_SIZE],
                                                      dtype=self.FLOAT_TYPE,
                                                      name='initial_state')
                self.past_state = tf.zeros([self.inf_batch_size, self.HIDDEN_LAYER_SIZE],
                                                      dtype=self.FLOAT_TYPE,
                                                      name='initial_state')
                ...
                s_state = tf.cond(tf.equal(self.mode, 1.0),
                                         lambda: s_state,
                        ...
                        with tf.variable_scope('state_recursion', reuse=tf.AUTO_REUSE):
                            ...
                            if self.DEC_CELL_TYPE == 'manual':
                                new_state = tf.tanh(
                                    ...
                                    tf.matmul(events_embedded, self.Wy) +
                                    (tf.matmul(delta_t_prev, self.Wdt) if type_delta_t else tf.matmul(time_2d, self.Wdt)) +
        ...
        for epoch in range(self.last_epoch, self.last_epoch + num_epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                if check_nans:
                    ...
                    if with_summaries:
                        _, summaries, cur_state, loss_, step, time_loss_, mark_loss_ = \
    ...
    def predict(self, event_in_seq, time_in_seq, decoder_length, single_threaded=False, plot_dir=False, event_out_seq=None):
        ...
        for dec_idx in range(0, decoder_length):
            ...
            all_decoder_states, all_event_preds, D, WT = self.sess.run(
                [self.decoder_states, self.event_preds, self.D, self.WT],
                feed_dict=feed_dict
            )
            ...
            time_pred_last = past_pred_time_infer
            if self.CONCAT_FINAL_ENC_STATE:
                all_decoder_states = np.concatenate([all_decoder_states, np.tile(np.expand_dims(cur_state, axis=1), [1, self.DEC_LEN, 1])], axis=-1)

            if single_threaded:
                current_time_preds = [_quad_worker((idx, (D_i, WT_i, state, t_last))) for idx, (D_i, WT_i, state, t_last) in enumerate(zip(D, WT, all_decoder_states, time_pred_last))]
            else:
                with MP.Pool() as pool:
                    current_time_preds = pool.map(_quad_worker, enumerate(zip(D, WT, all_decoder_states, time_pred_last)))

            all_time_preds.append(current_time_preds)

        all_time_preds = np.asarray(all_time_preds)
        all_event_preds_softmax = np.asarray(all_marker_preds)

        all_event_preds = np.argmax(all_event_preds_softmax, axis=-1) + 1

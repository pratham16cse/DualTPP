...
def minimize_func(t, c, w):
    """This function calculates the mode of the function f(t),
    given c, w."""
    return -1 *  c * np.exp(w * t + (c / w) * (-np.exp(w * t) + 1))
...
class RMTPP_DECRNN:
    ...
    def predict(self, event_in_seq, time_in_seq, decoder_length, single_threaded=False):
        ...
        all_hidden_states, all_event_preds, cur_state = self.sess.run(
        ...
        def _quad_worker(params):
            batch_idx, (all_decoder_states, time_pred_last) = params
            preds_i = []
            ...
            for pred_idx, s_i in enumerate(all_decoder_states):
                ...
                args = (c_, wt)
                bnds = ((0, None),)
                res = minimize(minimize_func, (0.01), args=args, bounds=bnds)
                val = res.x[0]
                #print(val, c_, t_last)
        ...
        if single_threaded:
            all_time_preds = [_quad_worker((idx, (state, time_pred_last))) for idx, (state, t_last) in enumerate(zip(all_decoder_states, time_pred_last))]
        else:
            with MP.Pool() as pool:
                all_time_preds = pool.map(_quad_worker, enumerate(zip(all_decoder_states, time_pred_last)))

...
class RMTPP_DECRNN:
        ...
        with tf.variable_scope(scope):
            with tf.device(device_gpu if not cpu_only else device_cpu):
                ...
                self.all_vars = [self.Wt, self.Wem, self.Wh, self.bh, self.Ws, self.bs,
                                 self.Wy, self.Vy, self.Vt, self.bt, self.bk, self.Wg, self.gamma]
                ...
                with tf.name_scope('Decoder'):
                    ...
                    D = tf.squeeze(tf.tensordot(self.decoder_states, self.Vt, axes=[[2],[0]]), axis=-1) + base_intensity - self.wt * gap_thresholds
                    D = -tf.nn.softplus(-D)
                ...
                with tf.name_scope('loss_calc'):
                    ...
                    self.loss += tf.sqrt(
                            tf.reduce_sum(
                                tf.square(
                                    tf.where(self.events_out > 0,
                                             gap_thresholds - self.times_out,
                                             tf.zeros(shape=(self.inf_batch_size, self.DEC_LEN)))
                                    )
                            )
                    ) / (tf.cast(self.inf_batch_size, tf.float32)*self.DEC_LEN*1.0)
    ...
    def predict(self, event_in_seq, time_in_seq, decoder_length, single_threaded=False):
        ...
        [Vt, bt, Wg]  = self.sess.run([self.Vt, self.bt, self.Wg])

        global _quad_worker
        def _quad_worker(params):
            batch_idx, (all_decoder_states, h_m, time_pred_last) = params
            preds_i = []

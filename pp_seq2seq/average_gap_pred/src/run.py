...
def_opts = average_gap_pred.average_gap_pred_core.def_opts
...
def cmd(dataset_name, alg_name, dataset_path,
    ...
    data = average_gap_pred.utils.read_seq2seq_data(
    ...
    def model_creator(params):
        ...
        #average_gap_pred.utils.data_stats(data) #TODO(PD) Need to support seq2seq models.

        hidden_layer_size, wt_hparam, restart, num_epochs, save_dir, train_eval = params
        average_gap_pred_mdl = average_gap_pred.average_gap_pred_core.AVERAGE_GAP_PRED(
            sess=sess,
            ...
            epsilon=epsilon,
            _opts=average_gap_pred.average_gap_pred_core.def_opts
        )

        return average_gap_pred_mdl

    def hyperparameter_worker(params, average_gap_pred_mdl, dec_len, restore_path=None):
        ...
        # saving the model.
        average_gap_pred_mdl.initialize(finalize=False)
        result = average_gap_pred_mdl.train(training_data=data, restart=restart,
    ...
    for dec_len in decoder_length_run:
        ...
        if os.path.isfile(save_dir+'/result.json'):
            ...
            average_gap_pred_mdl = model_creator(args)
            result = hyperparameter_worker(args, average_gap_pred_mdl, dec_len)
        else:
            ...
            for params in product(*hparams[alg_name].values()):
                ...
                average_gap_pred_mdl = model_creator(args)
                result = hyperparameter_worker(args, average_gap_pred_mdl, dec_len, checkp_dir)

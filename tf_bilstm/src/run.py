...
def cmd(event_train_file, time_train_file, event_test_file, time_test_file,
    ...
    print('BiLSTM begin')
    bilstm_model = tf_rmtpp.rmtpp_core.BiLSTM(
        sess=sess,
    ...
    print('BiLSTM end')
    print('begin init')
    bilstm_model.initialize(finalize=False)
    print('end init')
    print("training begin")
    bilstm_model.train(training_data=data, restart=restart,
    ...
    if train_eval:
        ...
        train_time_preds, train_event_preds = bilstm_model.predict_train(data=data)
        bilstm_model.eval(train_time_preds, data['train_time_out_seq'],
                       train_event_preds, data['train_event_out_seq'])
        print()

    if test_eval:
        print('\nEvaluation on testing data:')
        test_time_preds, test_event_preds = bilstm_model.predict_test(data=data)
        bilstm_model.eval(test_time_preds, data['test_time_out_seq'],

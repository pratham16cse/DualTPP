...
def get_preprocessed_(data, block_size, decoder_length, initial_timestamp):
    marks, times = data
...
def get_preprocessed_data(block_size, decoder_length):
    marks, times, initial_timestamp = read_data('testdata.txt')
    # '../pp_seq2seq/data/DataSetForSeq2SeqPP/Delhi.txt'
    #marks, times = split_data((marks, times), 7)
    
    block_size_sec = block_size * 3600.0

    data = get_preprocessed_((marks, times), block_size, decoder_length, initial_timestamp)

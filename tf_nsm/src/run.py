...
def cmd(event_train_file, time_train_file, event_test_file, time_test_file,
    ...
    if train_eval:
        ...
        train_time_preds, train_event_preds = nsm_model.predict_train(data=data)
        nsm_model.eval(train_time_preds, data['train_time_out_seq'],
                       train_event_preds, data['train_event_out_seq'])
        print()

    if test_eval:
        print('\nEvaluation on testing data:')
        test_time_preds, test_event_preds = nsm_model.predict_test(data=data)
        nsm_model.eval(test_time_preds, data['test_time_out_seq'],
